...
KLFitter::Fitter::Fitter()
        : fDetector(nullptr), fParticles(nullptr), ETmiss_x(0.), ETmiss_y(0.), SumET(0.), fParticlesPermuted(nullptr),
          ...
          fCustomPermutations(std::make_unique<KLFitter::CustomPermutations>(&fParticles, &fParticlesPermuted)),
...
int KLFitter::Fitter::SetParticles(KLFitter::Particles *particles, int nPartonsInPermutations) {
    ...
    if (fLikelihood && fLikelihood->getFPermMethod() == KLFitter::LikelihoodBase::kPermutations) {
...
//          fLikelihood->RemoveForbiddenParticlePermutations();
    ...
    } else if (fLikelihood && fLikelihood->getFPermMethod() == KLFitter::LikelihoodBase::kCustomPermutations) {

        fCustomPermutations->CreatePermutations();

        if (!fCustomPermutations->SetPermutation(0))
            return 0;
...
int KLFitter::Fitter::Fit(int index) {
    ...
    // set permutation
    if (fLikelihood->getFPermMethod() == KLFitter::LikelihoodBase::kPermutations) {
        if (!fPermutations->SetPermutation(index))
            return 0;
    } else if (fLikelihood->getFPermMethod() == KLFitter::LikelihoodBase::kCustomPermutations) {
        if (!fCustomPermutations->SetPermutation(index))
            return 0;
    }
    ...
    if (fLikelihood->getFPermMethod() == KLFitter::LikelihoodBase::kPermutations)
        ...
        fParticlesPermuted = fCustomPermutations->ParticlesPermuted(); // Todo: make the switch of permutation types nicer. with a permutation base class for example
    ...
    int nperms = 0;
    if (fLikelihood->getFPermMethod() == KLFitter::LikelihoodBase::kPermutations)
        nperms = fPermutations->NPermutations();
    else if (fLikelihood->getFPermMethod() == KLFitter::LikelihoodBase::kCustomPermutations)
        nperms = fCustomPermutations->NPermutations();
...
int KLFitter::Fitter::GetFitStatusFromCache(int iperm) {
    if ((static_cast<int>(fCachedConvergenceStatusVector.size()) > iperm) &&
        (static_cast<int>(fCachedMinuitStatusVector.size()) > iperm)) {
        fConvergenceStatus = fCachedConvergenceStatusVector.at(iperm);
        fMinuitStatus = fCachedMinuitStatusVector.at(iperm);
    } else {
        std::cout
                << "KLFitter::Fitter::GetFitStatusFromCache: size of fCachedConvergenceStatusVector or fCachedMinuitStatusVector too small!"
                << std::endl;
    }
...
int KLFitter::Fitter::SetFitStatusToCache(int iperm, int nperms) {
    ...
    if ((iperm > static_cast<int>(fCachedMinuitStatusVector.size())) ||
        (iperm > static_cast<int>(fCachedConvergenceStatusVector.size()))) {
        std::cout
                << "KLFitter::Fitter::SetFitStatusToCache: iperm > size of fCachedMinuitStatusVector or fCachedConvergenceStatusVector!"
                << std::endl;
        return 0;
    }
    ...
    if (partner > iperm) {
        if ((static_cast<int>(fCachedMinuitStatusVector.size()) > partner) &&
            (static_cast<int>(fCachedConvergenceStatusVector.size()) > partner)) {
            fCachedMinuitStatusVector.at(partner) = fMinuitStatus;
            fCachedConvergenceStatusVector.at(partner) = fConvergenceStatus;
        } else {
            std::cout
                    << "KLFitter::Fitter::SetFitStatusToCache: size of fCachedMinuitStatusVector or fCachedConvergenceStatusVector too small!"
                    << std::endl;
        }
    }
